...
function openDownloadModal(collectionName, price) {
    if (price === 'free') {
        // Для бесплатных коллекций - прямое скачивание
        downloadCollection(collectionName);
    } else {
        // Для платных - открыть модальное окно
        if (downloadModal && downloadDescription) {
            downloadDescription.textContent = `Коллекция "${collectionName}" доступна за ${price}`;
...
function downloadCollection(collectionName) {
    ...
    const demoContent = `
🎨 ${collectionName} - Pixel Art Collection

Спасибо за интерес к нашим ассетам!

Эта демо-версия содержит:
- Информацию о коллекции
- Ссылку на полную версию
- Контактные данные

Для получения полной коллекции свяжитесь с нами:
📧 orders@pixelartmaster.com
📱 Telegram: @pixelartmaster

---
Pixel Art Master Studio
Создаем качественные ассеты для инди-игр!
    `.trim();
