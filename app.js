...
function createCollectionCard(collection, isPremium) {
    ...
    const actionsHtml = isPremium ? 
        `<button class="btn btn--primary btn-sm" onclick="openPaymentModal('${collection.name}', '${collection.price}')">${collection.price}</button>` :
        `<button class="btn btn--primary btn-sm" onclick="openDownloadModal('${collection.name}', 'free')">Скачать</button>`;
    
    card.innerHTML = `
        <div class="collection-preview" onclick="openLightbox('${collection.preview}', '${collection.name}', '${collection.description}')">
            ...
            <div class="collection-placeholder" style="display: none;">🎨</div>
        </div>
        <div class="collection-info">
            <h3 class="collection-title">${collection.name}</h3>
            <p class="collection-description">${collection.description}</p>
            <div class="collection-meta">
                <span class="collection-count">${collection.count} ассетов</span>
...
// Модальные окна и функции скачивания
function openDownloadModal(collectionName, type) {
    if (type === 'free') {
        downloadCollection(collectionName);
    } else {
        if (downloadModal && downloadDescription) {
            downloadDescription.textContent = `Коллекция "${collectionName}" готова к скачиванию`;
...
function openPaymentModal(collectionName, price) {
    if (downloadModal && downloadDescription) {
        downloadDescription.innerHTML = `
            <h3>Покупка коллекции "${collectionName}"</h3>
            <p>Стоимость: ${price}</p>
            <form id="paymentForm" class="payment-form">
                <div class="form-group">
                    <label class="form-label">Email для получения коллекции</label>
            ...
            <p class="payment-note">После оплаты ссылка для скачивания будет отправлена на ваш email</p>
        `;
        downloadModal.classList.add('active');
        currentCollection = collectionName;
        
        // Добавляем обработчик формы оплаты
        const paymentForm = document.getElementById('paymentForm');
        if (paymentForm) {
            paymentForm.addEventListener('submit', function(e) {
                e.preventDefault();
                simulatePayment(collectionName);
            });
        }
    }
}

function simulatePayment(collectionName) {
    // Имитация оплаты
    closeDownloadModal();
    
    // Показываем сообщение об успехе
    if (successModal) {
        successModal.querySelector('.modal-description').textContent = `Оплата успешна! Ссылка на коллекцию "${collectionName}" отправлена на ваш email.`;
...
function downloadCollection(collectionName) {
    const demoContent = `Демо-версия коллекции: ${collectionName}\n\nДля получения полной версии напишите на: orders@pixelartmaster.com`;
...
function handleOrderSubmit(e) {
    ...
    if (submitOrderBtn) {
        ...
        setTimeout(() => {
            ...
            // Show success
            if (successModal) {
                successModal.querySelector('.modal-description').textContent = 'Ваша заявка успешно отправлена! Мы свяжемся с вами в ближайшее время.';
                successModal.classList.add('active');
            }
...
function addUploadedFileToList(file) {
    ...
    fileItem.innerHTML = `
        ...
        <button type="button" class="file-remove" onclick="removeUploadedFile('${file.name}')">✕</button>
    `;
...
console.log('🎨 Pixel Art Master - App initialized successfully!');
